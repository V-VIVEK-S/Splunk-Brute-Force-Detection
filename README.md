...
*Caption:* Audit policies enabled to log failed (4625) and successful (4624) logins on Windows VM.  
...
*Caption:* Universal Forwarder installed and running, configured to forward Windows Security logs to Splunk.  
...
*Caption:* Data input created in Splunk Web UI for Windows Event Logs, using custom `wineventlog` index.  
...
*Caption:* Hydra running RDP brute force attack generating failed login attempts on Windows VM.  
...
*Caption:* Splunk search results showing failed login attempts captured from Hydra attack logs.  
